...
class addSliderImage extends StatefulWidget {
  const addSliderImage({Key? key}) : super(key: key);

  @override
  State<addSliderImage> createState() => addSliderImageState();
}

class addSliderImageState extends State<addSliderImage> {
  XFile? imageXFile;
  var imageUrl;
  ...
  Addcategories() async {
    ...
    await storageService().uploadimage("${_sliderImageName.text}", imageXFile!.path.toString(), "Slider" );
    var data = await storageService().fetchImage("${_sliderImageName.text}", "Slider");
    setState(() {
